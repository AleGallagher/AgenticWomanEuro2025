...
class SQLAgent:
    ...
    def _not_found(self, state):
        ...
        combined_prompt = PromptTemplate.from_template(
            ...
            Rewrite the following question as a noun phrase that describes its topic:
            Question: {question}
            Description:
            """
        )
        response = llm.invoke(combined_prompt.format(
    ...
    def _create_reasoning_node(self):
        ...
        executor = AgentExecutor(agent=agent, tools=tools, tool_choice="auto", max_iterations=10)
    ...
    def _setup_sql_toolkit(self):
        global _cached_toolkit, _cached_prompt
        if _cached_toolkit and _cached_prompt:
            return _cached_toolkit, _cached_prompt
        system_message = """
                You are an expert PostgreSQL assistant. Your primary goal is to generate a single, comprehensive, syntactically correct SQL query to answer the user's question, prioritizing human-readable information using the following SQL database.
                Here is the current schema of your database:
                ### Tables and Columns:
        ...
        _cached_toolkit = toolkit.get_tools()
